...
mod unittest {
    ...
    fn test_crc32() {
        ...
        crc.sum_bytes(&buf, buf.len());
        assert_eq!(crc.get(), 0x29058c73);
        // use network bytes order for hdlc protocol, that's why we unpack to le bytes
        let crc32_swap: [u8; 4] = (crc.get()).to_le_bytes();
        crc.sum_byte(crc32_swap[0]);
        crc.sum_byte(crc32_swap[1]);
    ...
    fn test_crc16() {
        ...
        crc.sum_bytes(&buf, buf.len());
        assert_eq!(crc.get(), 0x303c);
        // use network bytes order for hdlc protocol, that's why we unpack to le bytes
        let crc16_swap: [u8; 2] = (crc.get()).to_le_bytes();
